...
def get_user_fav(user):
    ...
    all_fav_course = [Course.objects.get(
        id=user_fav_course.fav_id) for user_fav_course in all_user_fav_course]

    # 获取所有用户收藏的讲师
    all_user_fav_teacher = UserFavorite.objects.filter(user=user, fav_type=3)
    all_fav_teacher = [Teacher.objects.get(
        id=user_fav_teacher.fav_id) for user_fav_teacher in all_user_fav_teacher]

    # 收藏的机构
    all_user_fav_org = UserFavorite.objects.filter(user=user, fav_type=2)
    all_fav_org = [CourseOrg.objects.get(
        id=user_fav_org.fav_id) for user_fav_org in all_user_fav_org]
