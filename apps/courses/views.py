...
class CourseListView(View):
    def get(self, request):
        all_degree = list(
            map(lambda x: {'code': x[0], 'explain': x[1]}, Course.DEGREE_CHOICES))  # 显示难度等级
        ...
        hot_course = Course.objects.all().order_by(
            '-students')[:3]  # 热门课程选择3个显示
...
class CourseDetailView(View):
    def get(self, request, course_id):
        ...
        # 相关推荐
        course_tags_ids = course.tags.values_list(
            'id', flat=True)  # 获取当前课程所有标签的id
        similar_course = Course.objects.filter(
            tags__in=course_tags_ids).exclude(id=course.id)  # 获取包含这些标签的课程并排除当前课程
        similar_course = similar_course.annotate(same_tags=Count('tags')).order_by(
            '-same_tags', '-students')[:3]  # 使用Count聚合函数来生成一个计算字段same_tags

        # 学习该课程的用户，首先获取该课程在UserCourse对应关系，然后查询UserCourse表中的所有用户，使用distinct()去重
        user_courses = UserCourse.objects.filter(course=course)
        # print(user_courses)
        user_courses = user_courses.values(
            'user__nick_name', 'user__username', 'user__image').distinct()
        # print(user_courses)
...
class CourseContentView(LoginRequiredMixin, View):
    ...
    def get(self, request, course_id, **kwargs):  # kwargs 专用于获取视频id的
        ...
        has_user_course = UserCourse.objects.filter(
        ...
        related_user_course = UserCourse.objects.filter(
            user__in=all_user).exclude(course=course)
        # 获取所有关联课程的id
        related_course_id = [
            user_course.course.id for user_course in related_user_course]
        # 获取关联课程id在课程模型中的查询集，并且按照学习人数排序，取前5个
        related_course = Course.objects.filter(
            id__in=related_course_id).order_by('-students')[:5]
